...
val restoreAndroidAutoPlaylistsPatch = bytecodePatch(
    ...
    execute {
        ...
        patchNestedItemSections()
    }
}

private fun BytecodePatchContext.patchNestedItemSections() {
    val containerInterface = EXTENSION_CLASS.removeSuffix(";") + "\$NestedContainer;"
    val wrappedInterface = EXTENSION_CLASS.removeSuffix(";") + "\$WrappedRenderer;"
    val serializedInterface = EXTENSION_CLASS.removeSuffix(";") + "\$SerializedRenderer;"
    val elementType = ElementRendererProtoFingerprint.originalMethod.instructions
        .single { it.opcode == Opcode.CONST_CLASS }.getReference<TypeReference>()!!.type
    val createPlayableId = CreatePlayableMediaIdFingerprint.originalMethod
    val actionType = createPlayableId.parameterTypes.single().toString()
    val actionClass = classDefBy(actionType)
    val actionDefault = actionClass.fields.single { AccessFlags.STATIC.isSet(it.accessFlags) && it.type == actionType }
    val parseCommand = generateSequence(actionClass) { it.superclass?.let(::classDefByOrNull) }
        .flatMap { it.methods.asSequence() }.first { method ->
            AccessFlags.STATIC.isSet(method.accessFlags) && method.name == "parseFrom" &&
                method.parameterTypes.size == 3 && method.parameterTypes[1] == "[B" &&
                method.parameterTypes[2] == "Lcom/google/protobuf/ExtensionRegistryLite;"
        }
    mutableClassDefBy(elementType).apply {
        interfaces.add(serializedInterface)
        addInterfaceMethod(extensionInterfaceMethod(serializedInterface, "patch_getBytes"), 1, """
            invoke-virtual { p0 }, $elementType->toByteArray()[B
            move-result-object p0
            return-object p0
        """)
        addInterfaceMethod(extensionInterfaceMethod(serializedInterface, "patch_createPlayableId"), 4, """
            sget-object v0, $actionDefault
            invoke-static {}, Lcom/google/protobuf/ExtensionRegistryLite;->getGeneratedRegistry()Lcom/google/protobuf/ExtensionRegistryLite;
            move-result-object v1
            invoke-static { v0, p1, v1 }, $parseCommand
            move-result-object v0
            check-cast v0, $actionType
            invoke-static { v0 }, $createPlayableId
            move-result-object v0
            return-object v0
        """)
    }
    val protoType = ItemSectionProtoFingerprint.originalMethod.definingClass
    val wrapperType = ItemSectionConsumerFingerprint.originalMethod.instructions
        .filter { it.opcode == Opcode.CHECK_CAST }
        .mapNotNull { it.getReference<TypeReference>()?.type }
        .distinct().single { type -> classDefBy(type).fields.any { it.type == protoType } }
    val wrapper = mutableClassDefBy(wrapperType)
    val protoField = wrapper.fields.single { it.type == protoType && !AccessFlags.STATIC.isSet(it.accessFlags) }
    val rowsField = classDefBy(protoType).fields.single { it.name == "e" && it.type.startsWith("L") }
    // Protobuf schema records the element class immediately after the repeated field name.
    val schema = classDefBy(protoType).methods.single { it.name == "dynamicMethod" }
    val schemaInstructions = schema.instructions.toList()
    val rowsNameIndex = schemaInstructions.indexOfFirst {
        it.getReference<com.android.tools.smali.dexlib2.iface.reference.StringReference>()?.string == rowsField.name
    }
    if (rowsNameIndex < 0) throw PatchException("Missing item-section row schema")
    val rowType = schemaInstructions.drop(rowsNameIndex + 1).first { it.opcode == Opcode.CONST_CLASS }
        .getReference<TypeReference>()!!.type
    val decoder = Fingerprint(
        accessFlags = listOf(AccessFlags.PUBLIC, AccessFlags.STATIC),
        parameters = listOf(rowType),
        returnType = "Lcom/google/protobuf/MessageLite;",
        custom = { method, _ -> method.instructions.count() > 1000 },
    ).originalMethod
    wrapper.interfaces.add(containerInterface)
    wrapper.addInterfaceMethod(extensionInterfaceMethod(containerInterface, "patch_getChildren"), 1, """
        iget-object p0, p0, $protoField
        iget-object p0, p0, $rowsField
        return-object p0
    """)
    mutableClassDefBy(rowType).apply {
        interfaces.add(wrappedInterface)
        addInterfaceMethod(extensionInterfaceMethod(wrappedInterface, "patch_getRenderer"), 1, """
            invoke-static { p0 }, $decoder
            move-result-object p0
            return-object p0
        """)
    }
...
private fun BytecodePatchContext.patchPhoneBrowseResponses() {
    ...
    val songsPaginationDecoder = classDefBy(getOpenedPlaylistSongsMethod.definingClass).methods.single {
        AccessFlags.BRIDGE.isSet(it.accessFlags) && it.returnType == "Ljava/lang/Object;" &&
            it.parameterTypes.map(CharSequence::toString) == listOf(getPaginationResponseMethod.returnType)
    }
    val decodeMoreSongs = addPaginatedLibraryGridDecoder(songsPaginationDecoder)
    mutableClassDefBy(getTabsMethod.definingClass).addInterfaceMethod(
        extensionInterfaceMethod(EXTENSION_BROWSE_RESPONSE_INTERFACE, "patch_getMoreSongs"), 2, """
            invoke-virtual { p0 }, $getPaginationResponseMethod
            move-result-object p0
            const/4 v0, 0x0
            invoke-static { v0, p0 }, $decodeMoreSongs
            move-result-object p0
            check-cast p0, $EXTENSION_OPENED_PLAYLIST_SONGS_INTERFACE
            return-object p0
        """,
    )
}
...
private fun BytecodePatchContext.addOpenedPlaylistSongsInterface(
    ...
    val getActionsMethod = classDefBy(getSongsMethod.definingClass).methods.single {
        AccessFlags.STATIC.isSet(it.accessFlags) && it.returnType == "Ljava/util/List;" &&
            it.parameterTypes.map(CharSequence::toString) == listOf(openedPlaylistSongsType)
    }
    mutableClassDefBy(getActionsMethod.definingClass).findMutableMethodOf(getActionsMethod).apply {
        accessFlags = accessFlags.toPublicAccessFlags()
    }
    ...
    openedPlaylistSongsClass.addInterfaceMethod(
        extensionInterfaceMethod(EXTENSION_OPENED_PLAYLIST_SONGS_INTERFACE, "patch_getContinuationActions"), 1, """
            invoke-static { p0 }, $getActionsMethod
            move-result-object p0
            return-object p0
        """,
    )
...
private fun BytecodePatchContext.patchPlaylistOrTrack() {
    ...
    val hasBrowseEndpointMethod = actionToBrowseEndpointMethod.instructions
        .mapNotNull { it.getReference<MethodReference>() }
        .distinct().single {
            it.returnType == "Z" && it.parameterTypes.map(CharSequence::toString) == listOf(playlistOrTrackActionType)
        }
...
private fun MutableClass.addPlaylistBrowseIdGetter(
    ...
    hasBrowseEndpointMethod: MethodReference,
) {
    addInterfaceMethod(
        ...
        instructions = """
            ...
            invoke-static { v1 }, $hasBrowseEndpointMethod
            move-result v2
...
private fun BytecodePatchContext.addAndroidAutoPlaylistsRequestInterface(
    ...
    // 9.15.51's LoadChildrenResult has one instance boolean, initialized by the
    // native root-membership check. Reject other layouts instead of guessing.
    val rootField = androidAutoRequestClass.fields.single { field ->
        field.type == "Z" && !AccessFlags.STATIC.isSet(field.accessFlags)
    }
    androidAutoRequestClass.addInterfaceMethod(
        extensionInterfaceMethod(EXTENSION_ANDROID_AUTO_PLAYLISTS_REQUEST_INTERFACE, "patch_isRoot"),
        1,
        """
            iget-boolean p0, p0, $rootField
            return p0
        """,
    )
    androidAutoRequestClass.addInterfaceMethod(
